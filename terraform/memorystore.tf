...
resource "null_resource" "kustomization-update" {
  provisioner "local-exec" {
    ...
    command     = "sed -i \"s/REDIS_CONNECTION_STRING/${google_redis_instance.redis-cart[0].host}:${google_redis_instance.redis-cart[0].port}/g\" \"${var.filepath_manifest}/components/memorystore/kustomization.yaml\""
  }
